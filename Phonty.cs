...
namespace PhontyPlus {
    public class Phonty : NPC, IClickable<int> {
        ...
        public static void LoadAssets() {
            ...
            try { deafIcon = AssetLoader.SpriteFromTexture2D(AssetLoader.TextureFromMod(Mod.Instance, "Textures", "Player_Deaf_Icon.png"), 50f); }
            catch { deafIcon = idle; }
            ...
            for (int i = 0; i < 6; i++) emergeFrames.Add(emergeSheet2[i]);
            ...
            foreach (var path in recordsFolder) records.Add(ObjectCreators.CreateSoundObject(AssetLoader.AudioClipFromFile(path), "Phonty_Vfx_Record", SoundType.Voice, Color.yellow));
        }
        ...
        protected override void VirtualUpdate() {
            if (deafPlayer && AudioListener.volume > 0.01f) AudioListener.volume = 0.01f;
        }
        ...
        public void Clicked(int player) { if (!angry) ResetTimer(); }
        public void ResetTimer() { behaviorStateMachine.ChangeState(new Phonty_PlayingMusic(this, true)); }
        ...
        private IEnumerator DeafenPlayer() {
            ...
            if (hud != null)
            {
                ...
                if (gaugeManager != null) gauge = gaugeManager.ActivateNewGauge(deafIcon, duration);
            }
            deafPlayer = true;
            StartCoroutine(SubtitlesAlpha(0.01f, 2f));
            AudioListener.volume = 0.01f;
            if (Mod.GlobalMixer != null) Mod.GlobalMixer.SetFloat("EchoWetMix", 1f);
            while (timer > 0f)
            {
                timer -= Time.deltaTime;
                if (gauge != null) gauge.SetValue(duration, timer);
                yield return null;
            }
            deafPlayer = false;
            AudioListener.volume = 1f;
            if (Mod.GlobalMixer != null) Mod.GlobalMixer.SetFloat("EchoWetMix", 0f);
            StartCoroutine(SubtitlesAlpha(1f, 2f));
            if (gauge != null) gauge.Deactivate();
        }
    ...
    public class Phonty_StateBase : NpcState {
        ...
        public Phonty_StateBase(Phonty phonty) : base(phonty) { this.phonty = phonty; }
    }
    ...
    public class Phonty_PlayingMusic : Phonty_StateBase {
        ...
        public override void Enter() {
            ...
            if (interacted) phonty.audMan.PlaySingle(Phonty.audios.Get<SoundObject>("windup"));
        ...
        public override void Update() {
            base.Update();
            if (timeLeft <= 0) phonty.behaviorStateMachine.ChangeState(new Phonty_Chase(phonty));
            else timeLeft -= Time.deltaTime * phonty.ec.NpcTimeScale;
    ...
    public class Phonty_Chase : Phonty_StateBase {
        ...
        public override void OnStateTriggerEnter(Entity otherEntity, Collider other, bool validCollision) {
            ...
            if (pm != null) phonty.EndGame(pm.transform);
        }
        ...
        private IEnumerator Emerge() {
            while (phonty.audMan.QueuedAudioIsPlaying) yield return null;
            if (!(phonty.behaviorStateMachine.currentState is Phonty_Chase)) yield break;
